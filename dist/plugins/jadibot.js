...
import { db, setBotSettings, getPrefix } from "../lib/db.js";
const restartingSub = {};
const reconnectTimers = {};
const reintentos = {};
const msgStore = new Map();
const cod_enviado = {};
const activeSubSockets = {};
const subbotLocks = new Set();
const subbotStarting = new Set();
const announcedOpen = new Set();
export const subbotUptimes = {};
function getCurrentServerIdentifier() {
    return String(process.env.P_SERVER_UUID ||
...
function hasLocalSubbotSession(numero) {
    return fs.existsSync(path.resolve(`./sessions/sub_${numero}/creds.json`));
}
function canCreateMoreSubbots(numero) {
    if (hasLocalSubbotSession(numero))
        return true;
...
async function saveSubbotInstance({ ownerId, phone, botId, dir }) {
...
async function deleteSubbotInstance(phone, botId) {
...
function destroySubSocket(numero) {
...
function clearReconnectTimer(numero) {
    if (reconnectTimers[numero]) {
        clearTimeout(reconnectTimers[numero]);
        reconnectTimers[numero] = null;
    }
}
function scheduleReconnect(parentSock, m, numero, delay = 5000) {
...
export async function iniciarSubbot(sock, m, isInitial = true, generaCodigo = true) {
    ...
    try {
        ...
        subSock.ev.on("connection.update", async ({ connection, lastDisconnect, isNewLogin, qr }) => {
            ...
            if (connection === "open") {
                ...
                const info = getLimitInfo();
                const tipo = (info.serverName === "free-1" ||
        ...
        subSock.ev.on("messages.upsert", async ({ messages, type }) => {
            ...
            for (const msg of messages) {
                ...
                if (msg.messageTimestamp && now - Number(msg.messageTimestamp) > 45)
                    continue;

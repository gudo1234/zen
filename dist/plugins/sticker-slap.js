...
export default {
    name: ["slap", "cachetada", "bofetada"],
    tags: ["sticker"],
    help: ["slap"],
    desc: "Dar una cachetada",
    register: true,
    run: async ({ conn, m }) => {
        let tmpIn = "";
        let tmpOut = "";
        ...
        try {
            ...
            const texto = `👋 ${senderName} le dio una cachetada a ${targetName}`;
            const waifuRes = await fetch("https://api.waifu.pics/sfw/slap");

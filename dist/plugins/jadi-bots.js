...
function getParticipantNumber(p) {
...
export default {
    ...
    tags: ["jadibot"],
    run: async ({ conn, m }) => {
        const mains = await db.query(`SELECT bot_id, tipo
       FROM bot_settings
       WHERE tipo = 'main'`);
        const subs = await db.query(`SELECT phone, plan, ptero_server_id
       FROM subbot_instances
       WHERE status = 'active'`);
        const mainIds = mains.rows
            .map((r) => cleanNum(r.bot_id))
            .filter(Boolean);
        let free = 0;
        let temporal = 0;
        let premium = 0;
        for (const row of subs.rows) {
            ...
            if (plan === "premium")
                ...
                temporal++;
            else if (isFreeServer(row.ptero_server_id))
                free++;
        }
        const freeServers = [
            {
        ...
        for (const s of freeServers) {
            const used = subs.rows.filter((r) => {
                const id = cleanNum(r.phone);
                if (!id || mainIds.includes(id))
                    return false;
                return (String(r.ptero_server_id) === s.id ||
